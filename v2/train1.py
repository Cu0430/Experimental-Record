import torch
from loss1 import ContrastiveLoss,IncrementalFalseNegativeDetection,L_Grad,CorrelationCoefficient,Loss_intensity
...
from dataset1 import MyData1, _create_model_training_folder # 自定义数据集类
...
class Trainer:
    def __init__(self, base_ms,base_pan,FE_ms, FE_pan, decoder, PAFE,MN_ms,MN_pan,
        ...
        _create_model_training_folder(self.writer, files_to_same=["/root/autodl-tmp/config/config.yaml", "/root/autodl-tmp/train1.py"])
...
# 将批次中的前 32 张图像通过 make_grid 转换为网格图像并写入 TensorBoard，以便在训练过程中可视化图像
                ...
                ms_1=self.base_ms(ms) #[128,8,16,16]
                pan_1=self.base_pan(pan) #[128,8,64,64]
                ms_2=self.PAFE(ms_1) #[128,8,16,16]
                pan_2=self.PAFE(pan_1) #[128,8,64,64]
                conv_resize = nn.Conv2d(in_channels=8, out_channels=8, kernel_size=1)
                
                # 上采样 MS
                ms_2_upsampled = F.interpolate(ms_2, size=(64, 64), mode='bilinear', align_corners=True)
                conv_resize = conv_resize.to(ms_2_upsampled.device)
                ms_2_transformed = conv_resize(ms_2_upsampled) 
                simple_fusion=ms_2_transformed+pan_2 #[128,8,64,64]
                fusion_image,fusion_f=self.decoder(simple_fusion)  #[128,4,64,64][128,8,64,64]
                ms_3=self.MN_ms(ms_2) #torch.Size([128, 8,16,16])
                pan_3=self.MN_pan(pan_2) #torch.Size([128, 8, 64, 64])

                MSDP_ms,_=self.MSDP(ms_3) #torch.Size([128, 8, 16,16])
                PANDP_pan,_=self.PANDP(pan_3)  #torch.Size([128, 8, 64, 64])

                ms3,ms4,ms5,_=self.FE_ms(MSDP_ms)
                
                pan3,pan4,pan5,_=self.FE_pan(PANDP_pan)
                ...
                MSDP_ms_r = F.interpolate(MSDP_ms, size=(64, 64), mode='bilinear', align_corners=False)
                ...
                if niter % 50000 :
                    ...
                    ms_features = ms_3.detach().cpu().numpy().reshape(len(ms_3), -1)
                    pan_features = pan_3.detach().cpu().numpy().reshape(len(pan_3), -1)
                    ms5_features = ms5.detach().cpu().numpy().reshape(len(ms5), -1)
                    pan5_features = pan5.detach().cpu().numpy().reshape(len(pan5), -1)
                    ...
                    self.writer.add_embedding(ms_features,  global_step=niter, tag="ms_3")
                    self.writer.add_embedding(pan_features,  global_step=niter, tag="pan_3")
                    self.writer.add_embedding(ms5_features,  global_step=niter, tag="ms5")
                    self.writer.add_embedding(pan5_features,  global_step=niter, tag="pan5")
    ...
    def save_model(self, PATH):
        ...
        torch.save({
            'base_ms_state_dict': self.base_ms.state_dict(),
            'base_pan_state_dict': self.base_pan.state_dict(),
            'FE_ms_state_dict': self.FE_ms.state_dict(),
            'FE_pan_state_dict': self.FE_pan.state_dict(),
            'decoder_state_dict': self.decoder.state_dict(),
            'PAFE_state_dict': self.PAFE.state_dict(),
            'MN_ms_state_dict': self.MN_ms.state_dict(),
            'MN_pan_state_dict': self.MN_pan.state_dict(),
...
def my_main():
    ...
    base_ms=model1.base_ms().to(device)
    base_pan=model1.base_pan().to(device)
    PAFE=model1.FeatureExtractor().to(device)
    MN_ms = model1.Enhance().to(device)
    MN_pan = model1.Enhance().to(device)

    FE_ms = model1.MS_Model().to(device)
    FE_pan = model1.PAN_Model().to(device)

    decoder = model1.Decoder().to(device)
    
    MSDP = model1.DictionaryRepresentationModule().to(device)
    PANDP = model1.DictionaryRepresentationModule().to(device)

    optimizer = torch.optim.Adam(list(FE_ms.parameters()) + list(decoder.parameters()) + list(base_pan.parameters())+
                                list(FE_pan.parameters()) + list(MSDP.parameters()) + list(base_ms.parameters())+
                                list(PANDP.parameters())+list(MN_pan.parameters())+list(MN_ms.parameters())+
                                list(PAFE.parameters()), **config['optimizer']['params'])
    """
    torch.nn.utils.clip_grad_norm_(list(FE_ms.parameters()) + list(decoder.parameters())  + list(base_pan.parameters())+
                                list(FE_pan.parameters())  + list(MSDP.parameters()) +list(base_ms.parameters())+
                                list(PANDP.parameters())+list(MN_pan.parameters())+list(MN_ms.parameters())+
                                list(PAFE.parameters()), max_norm=1.0)
    """
    scheduler = CosineAnnealingWarmRestarts(optimizer, T_0=10, T_mult=2)
    #每 5 个 epoch 就进行一次学习率重启，使用余弦退火策略调整学习率

    trainer = Trainer(base_ms=base_ms,base_pan=base_pan,FE_ms=FE_ms, FE_pan=FE_pan, decoder=decoder, 
                      PAFE=PAFE,MN_ms=MN_ms,MN_pan=MN_pan,
