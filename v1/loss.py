...
class CorrelationCoefficient(nn.Module):
    ...
    def c_CC(self, A, B): #A=[128,8,3,3] B=[128,8,2,2]
        
        #A_mean = torch.mean(A, dim=[2, 3], keepdim=True) #[128,8,1,1]
        #B_mean = torch.mean(B, dim=[2, 3], keepdim=True) #[128,8,1,1]
        #A_sub_mean = A - A_mean #[128,8,3,3]
        #B_sub_mean = B - B_mean #[128,8,2,2]
        sim = torch.sum(torch.mul(A, B))
