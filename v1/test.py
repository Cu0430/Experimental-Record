...
pretrained_folder = 'Mar31_15-24-38_autodl-container-c5e34fba27-153f56e2'


class Net(nn.Module):
    def __init__(self, num_classes=10):
        super(Net, self).__init__()
        FE_ms=model.MS_Model()
        FE_pan=model.PAN_Model()
        #decoder
        MSDP=model.DictionaryRepresentationModule1()
        PANDP=model.DictionaryRepresentationModule1()
        ...
        self.add_block = nn.Sequential(
            nn.Linear(4096, 2048),  # 输入维度匹配4096
            nn.BatchNorm1d(2048),
            nn.ReLU(True),
            ...
            nn.Linear(128, num_classes)  # 最终输出类别数
        )
        self.FE_ms=FE_ms
        self.FE_pan=FE_pan
    ...
    def forward(self, ms, pan):
        ...
        _,_,_,ms=self.FE_ms(ms)
        _,_,_,pan=self.FE_pan(pan)
        
        ms=self.MSDP(ms)
        pan=self.PANDP(pan)
        
        feature = torch.cat([ms, pan], 1)
