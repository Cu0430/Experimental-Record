...
class Trainer:
    ...
    def train(self, train_dataset):
        ...
        for epoch_counter in range(self.max_epochs):
            ...
            contrastive_loss.current_epoch = epoch_counter + 1
            contrastive_loss.total_epochs = self.max_epochs
...
# 将批次中的前 32 张图像通过 make_grid 转换为网格图像并写入 TensorBoard，以便在训练过程中可视化图像
                ...
                ms3,ms4,ms5,ms3_1,ms4_1,ms5_1=self.FE_ms(MSDP_ms) #[128,128]
                
                pan3,pan4,pan5,pan3_1,pan4_1,pan5_1=self.FE_pan(PANDP_pan) #[128,128]
                                              
                if niter % 500 == 0:
                    # 基础层特征 (MS需要上采样4倍)
                    self._visualize_stage(ms_1, pan_1, "01_Base", niter, ms_scale=4)
                    
                    # PAFE后特征
                    self._visualize_stage(ms_2, pan_2, "02_PAFE", niter, ms_scale=4)
                    
                    # 多模态增强后
                    self._visualize_stage(ms_3, pan_3, "03_MN_Enhanced", niter, ms_scale=4)
                    
                    # 字典投影后
                    self._visualize_stage(MSDP_ms, PANDP_pan, "04_Dictionary", niter, ms_scale=4)

                    self._visualize_stage(ms3_1, pan3_1, "05_Stage3", niter, ms_scale=4)
                    self._visualize_stage(ms4_1, pan4_1, "06_Stage4", niter, ms_scale=4)
                    self._visualize_stage(ms5_1, pan5_1, "07_Stage5", niter, ms_scale=2)
                    # 各层次特征
                    self.visualize_alignment_tsne(ms3, pan3, "05_Stage3", niter)
                    self.visualize_alignment_tsne(ms4, pan4, "06_Stage4", niter)
                    self.visualize_alignment_tsne(ms5, pan5, "07_Stage5", niter)
